...
with open(dir+"/iterdist.txt",'r') as f:
    lines = f.readlines()
    for i in range(len(lines)):
        if i%2 == 0:
            params = lines[i]
            rfdist=float(lines[i+1].strip())
            #print(params)
            ps = params.split(',')
            param = ps[1].strip()
            ps = param.split('/')
            ...
            k1 = 0
            if 'A' in split[1]:
                k1 = int(split[1].replace('KA',''))
            else:
                k1 = int(split[1].replace('K',''))
            runs.append((k1,rfdist))
...
print(x)
print(y)
plt.plot(x,y,label="Length=1500")
plt.legend()
plt.title('Distance from previous Iteration')
plt.show()
plt.xlabel('# genes')
plt.ylabel("distance")
plt.savefig(dir+'/rfdistances.png')
plt.clf()
with open(dir+"/iterdist2.txt",'r') as f:
    lines = f.readlines()
    for i in range(len(lines)):
        if i%2 == 0:
            params = lines[i]
...
plt.xlabel('# genes')
plt.ylabel("distance")
plt.savefig(dir+'/rfdistances2.png')
